# Basic settings
#
# I would recommend not using your final passwords in a plain-text file;
...
d-i partman-auto/disk string my_install_target
...
# NOTE: insecure, but will be replaced before sensitive data is placed on drive
# (note: a stronger password would prevent one type of evil maid attack, but
# would not prevent evil maid attacks in general.)
d-i partman-crypto/passphrase password my_luks_password

### Description: Re-enter passphrase to verify:
# NOTE: insecure - see above
d-i partman-crypto/passphrase-again password my_luks_password
...
#                      method{ swap } format{ }                \
#              .
d-i partman-auto/choose_recipe select full-disk-encryption
d-i partman-auto/expert_recipe string                         \
...
d-i grub-installer/bootdev  string my_install_target
...
d-i preseed/late_command string in-target mkdir /root/.ssh ; \
    ...
    cp /cdrom/preseed/authorized_keys /target/root/.ssh/authorized_keys ; \
    in-target chown -R root:root /root/.ssh ; \
    echo 'DROPBEAR_OPTIONS="-p 23 -s -j -k"' >> /target/etc/dropbear-initramfs/config ; \
    cat /cdrom/preseed/authorized_keys | \
